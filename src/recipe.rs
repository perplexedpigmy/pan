...
pub struct Recipe {
    ...
    total_flour_weight: f32,
    total_water_weight: f32,
    salt_weight: f32,
    salt_percentage: f32,
...
impl Recipe {
    const DEFAULT_TOTAL_FLOUR_WEIGHT: f32 = 600.0;  // In grams
    const DEFAULT_HYDRATION_PERCENTAGE: f32 = 0.7;  // 70% hydration
    const DEFAULT_SALT_PERCENTAGE: f32 = 0.02;      // 2% salt content
    const DEFAULT_STARTER_PERCENTAGE: f32 = 0.1;    // 10% starter of flour weight
    const DEFAULT_STARTER_HYDRATION: f32 = 1.0;     // 100% hydration
    ...
    pub fn default() -> Recipe {
        let hydration_percentage = Recipe::DEFAULT_HYDRATION_PERCENTAGE;
        let salt_percentage = Recipe::DEFAULT_SALT_PERCENTAGE;
        let total_flour_weight = Recipe::DEFAULT_TOTAL_FLOUR_WEIGHT;
        let total_water_weight = total_flour_weight * hydration_percentage;
        let starter_percentage = Recipe::DEFAULT_STARTER_PERCENTAGE;
        ...
        let starter = Starter::create(total_weight_excl_salt, Recipe::DEFAULT_STARTER_HYDRATION, starter_percentage);
    ...
    pub fn set_total_flours(&self, total_flour_weight: f32, flours: &Vec<FlourItem>) -> Recipe {
        let (starter_hydration, starter_percentage)= match self.get_starter() {
            Some(s) => (s.get_hydration(), self.starter_percentage),
            None => (Recipe::DEFAULT_STARTER_HYDRATION, Recipe::DEFAULT_STARTER_PERCENTAGE),
        };
        ...
        let mut ingredients: Vec<Ingredient> = flours.iter().map(|f| Ingredient::Flour(Flour{
            name: f.name.clone(), 
            weight: Gram::from(f.percentage * total_flour_weight - seq.next().unwrap_or(0.0))
        })).collect();
